...
// 三个时段。晨雾把远处的山化开，黄昏给鄱阳湖一层橙光，正午最适合看地形本身。
export const LIGHTS = [
  {
    id: 'morning', name: '晨雾',
    skyTop: 0x9fc3dd, skyBottom: 0xe6dcc8, fog: 0xd7dcd2, fogDensity: 1.4,
    sun: 0xffe3bb, sunIntensity: 1.5, azim: 0.9, elev: 0.28, ambient: 0.85, exposure: 1.02,
  },
  {
    id: 'noon', name: '正午',
    skyTop: 0x4c8fd6, skyBottom: 0xbcd8ee, fog: 0xc9dced, fogDensity: 0.7,
    sun: 0xfff6e2, sunIntensity: 2.1, azim: 2.1, elev: 0.75, ambient: 0.95, exposure: 1.0,
  },
  {
    id: 'dusk', name: '黄昏',
    skyTop: 0x2e3f66, skyBottom: 0xf0a86a, fog: 0x8b7e8c, fogDensity: 1.1,
...
/**
 * 视角预设。region 的那几个把镜头挪到该分区上方，id 与 taxonomy.js 的分区一致。
 * pitch 是抬头角（弧度），越大越接近正俯视；全省视角要压得高一些，
 * 否则江西南北 660 公里的纵深在 50° 视场里根本装不进一屏。
 */
const REGION_VIEWS = {
  // 左边有一块筛选面板压着画面，所以全省视角的中心往东挪一点，省域才不会被面板盖住；
  // 目标点再往北抬一些 —— 斜着看时近处（赣南）会被放大、远处（赣北）被压扁，
  // 瞄在几何中心上，南端反而会掉出画面。
  overview: { lng: 116.35, lat: 27.85, dist: 1.0, pitch: 1.05 },
  lake: { lng: 116.05, lat: 29.10, dist: 0.34, pitch: 0.72 },
  northeast: { lng: 117.45, lat: 28.75, dist: 0.36, pitch: 0.72 },
  west: { lng: 114.45, lat: 27.70, dist: 0.34, pitch: 0.72 },
  central: { lng: 115.65, lat: 27.35, dist: 0.38, pitch: 0.72 },
  south: { lng: 115.20, lat: 25.55, dist: 0.40, pitch: 0.70 },
  flat: { lng: 116.15, lat: 27.35, dist: 1.02, pitch: 1.48 },
};
...
export function createScene(canvas, { spots, onPick, onHover } = {}) {
  ...
  const setView = (id, immediate = false) => {
    ...
    const pos = new THREE.Vector3(target.x, target.y + dist * Math.sin(v.pitch), target.z + dist * Math.cos(v.pitch));
    flyTo(pos, target, immediate);
  };

  /** 飞到某个景点：从东南方压低一点看过去，比正上方更容易看出它靠山还是靠水 */
  const focusSpot = (id, dist = 230) => {
  ...
  const frame = (now) => {
    ...
    atlas.animate(clock, camera, canvas.clientWidth || window.innerWidth, canvas.clientHeight || window.innerHeight);
    controls.update();
    ...
    if (onStats && fps.length === 40 && ((clock * 10) | 0) % 8 === 0) {
