...
- `shici-jiangshan` 诗词江山图 — **不是游戏**，是一张《千里江山图》矿物色的 3D 诗词地图：唐宋元 334 首诗词曲、一枚印章管一处地方（120 处）、9 条诗人行迹、3 朝 8 主题筛选 — **不打算加音效**（同上）
...
- `shici-jiangshan`：3D 诗词地图《诗词江山图》，把丽水那套矿物色画法放到全国——**一枚印章管一处地方，不是一首诗**，唐宋元 334 首落在 120 处（玉门关到儋州、上京草原到岭南），印章的颜色是朝代、印文是主题，一处压着好几首时印章更大、点开先出这一处的清单（长安三十四首、杭州二十首、汴京十二首、洛阳与扬州各十一首）；诗名题签上写的是"地名 · N 首"，筛选之后数目会跟着改。归堆在判定层（`atlas/clusters.js`）：先按 0.12 度网格分，再把同名近邻并起来（格子边界会把长安切成两半），**分堆只取决于坐标、与数据表顺序无关**，所以印章 id 能直接当拾取标识；晕圈、杆与拾取体都是 `InstancedMesh`，印章要上到几百枚才不会被 draw call 拖死。条目从一百多首扩到一百七十多首时冒出一件只在密度上来后才看得见的事：**`place` 的首段就是这处地方的身份**，写成"长安市""长安里巷""长安南郊"会让长安散成三四枚印章，其中一枚还被邻近的乐游原抢了名字（于是《无题》的题签上写着"乐游原"），所以细部一律写在 `·` 之后（"长安·西市"），并由两条测试守着：**报同一个地名的诗必须落在同一枚印章上**、**一枚印章里的诗必须都报同一个首段**（否则印章刻谁的名字就成了众数碰运气——金陵那一枚曾因此刻着"石头城"）。这两条测试顺带翻出三处坐标错：《夏日绝句》的乌江偏了一百公里、《竹枝词》标夔州却落到一百四十公里外、《望天门山》挤进了乌江镇那一格。《静夜思》《锦瑟》这类**写作地历来无定说的，宁可不落点**（硬派一个地方就是编造），只进列表与检索；**落不下还有第二种理由 —— 地方认得出但在图幅之外**（这卷子西边止于玉门关一带，岑参的北庭、李颀的交河、王安石《明妃曲》的塞北毡城都在画外），两种都由同一条测试守着"必须在 place 里交代清楚"。放大到全国之后另有两件事必须重做：一是**色带不能一路加深**，横断山、青藏东缘一大片都在三千米以上，石青加深下去比海还暗、西半边会读成一片海湾，所以两千四以上改成越高越淡、最后淡进绢底（画里的"高则明、远则淡"），这条规则由 `atlas/palette.js` 的测试守着；二是**垂直夸张要按图幅重算**（省域图的 60 倍在这里会把秦岭拉成一百八十公里高，全图糊成一张皱铝箔，压到 30 倍）。一百多首一起写出题签就是一堵字墙，所以**全卷远看只出印章与古地名**，走近、点中、或那首诗在当前行迹上，诗名才铺开。数据按选集分册（`spots.js` 是最早那批，其后唐宋元各有 `spots-*-b/c.js`，末尾并成唯一的 `SPOTS`）；分批灌数据最容易犯的错是"同一首收了两遍、只是 id 起得不一样"，id 唯一性拦不住它，所以另有一条测试按"作者《篇名》"与正文前十四字各查一遍。元曲占 50 首，散曲的口气和唐诗宋词明显不同（睢景臣让一个乡下人把汉高祖的威仪拆了，萨都剌平视草原写乳酪和毡帘，无名氏《醉太平》张口就骂"堂堂大元"，卢挚与珠帘秀一赠一答成对收在扬州江上）。9 条行迹（李白／杜甫／苏轼／白居易与江州／边塞一线／金陵怀古／南渡之路／南贬之路／元曲行路），能搜作者、地名，也能搜诗句里的字。放大到全国之后另有两件事必须重做：一是**色带不能一路加深**，横断山、青藏东缘一大片都在三千米以上，石青加深下去比海还暗、西半边会读成一片海湾，所以两千四以上改成越高越淡、最后淡进绢底（画里的"高则明、远则淡"），这条规则由 `atlas/palette.js` 的测试守着；二是**垂直夸张要按图幅重算**（省域图的 60 倍在这里会把秦岭拉成一百八十公里高，全图糊成一张皱铝箔，压到 30 倍）。一百多首一起写出题签就是一堵字墙，所以**全卷远看只出印章与古地名**，走近、点中、或那首诗在当前行迹上，诗名才铺开。元曲占 26 首，散曲的口气和唐诗宋词明显不同（睢景臣让一个乡下人把汉高祖的威仪拆了，萨都剌平视草原写乳酪和毡帘）。9 条行迹（李白／杜甫／苏轼／白居易与江州／边塞一线／金陵怀古／南渡之路／南贬之路／元曲行路），能搜作者、地名，也能搜诗句里的字。
