...
22 个游戏 + 3 张 3D 旅游导览图（`zhejiang-luxing`、`jiangxi-luxing`、`lishui-luxing`，见下）+ 2 张长卷（`qiannian-jiangshan` 千年江山图、`shici-jiangshan` 诗词江山图）。「关卡」一列区分三种结构：**闯关**有关卡表，**无尽/单局**用难度或阶段代替关卡，**生成器**用风格预设代替关卡。

「音效」一列是持续在补的一项：11 个闯关游戏、方块坠塔、方块疾风、小队枪战、疯狂打弹珠、蜘蛛纸牌和魔方爬楼已经接上程序化音效、触觉反馈和奖励反馈，
...
- `huanyi-s-jihua` 换翼S计划 — 同样 16 关，判定层与换翼计划逐字节相同，**表现层换成 three.js 的 2.5D 斜俯视战场**（固定俯角 58° 的霓虹峡谷，往屏幕上方走就是往远处走） — **有音效 + 触感 + 奖励反馈**（与 2D 版共用同一套 `audio.js`）
...
- `shici-jiangshan` 诗词江山图 — **不是游戏**，是一张《千里江山图》矿物色的 3D 诗词地图：唐宋元 442 首诗词曲、一枚印章管一处地方（136 处）、12 条诗人行迹、3 朝 8 主题筛选 — **不打算加音效**（同上）
...
- `huanyi-s-jihua`：手机触屏纵版飞行射击《换翼S计划》，玩法与《换翼计划》完全同一套（`src/game/` 逐字节相同、同一个机器人跑完 16 关），换的是**整层画面：three.js 的 2.5D 斜俯视战场**。100×150 的场地摊成地面、相机架在近端上方压 58° 俯角，于是「往屏幕上方走就是往远处走」，敌人从地平线那头压过来——纵版射击里飞机其实钉在屏幕上，动的是世界，所以速度感交给地面栅格、栏上刻度、两侧岩体三层各跑一个速度。**取景是算出来的而不是调出来的**：`src/scene/view.js` 不 import three，所以「八种屏幕比例下场地的八个角都在画面内且贴着边」和「远近尺寸比夹在 0.5—0.85（高了是平面图、低了远处糊成一团）」这两条能在 `node --test` 里守住。配色同理搬进判定得到测试的位置（`palette.js`）：**每章亮色饱和度 > 0.55、四章色相互相拉开 40° 以上、地形色一律避开敌弹那个红 30° 以上**——敌弹是全场唯一碰到就掉翼的东西，第四章虽是红色调，亮色也只能走琥珀橙。画面上每一样东西都对应一条玩家需要知道的事：带翼的机身明显更宽（那截就是多出来的受击面积）、**阶级用三颗灯珠而不是刻字**（十几像素高的机身上，`C` 和 `III` 只会糊成两个白斑）、挨打的无敌是白环／弃翼的下潜是蓝环加一串残影、弱点环照 `weakSpots()` 摆、菱形道具能打进本关弱点就描金环否则整枚压暗。泛光是这套霓虹画法的一半，但半径必须压住：0.62 下 Boss 那种又大又亮的实体会把整条天空糊成一块青绿。
...
- `shici-jiangshan`：3D 诗词地图《诗词江山图》，把丽水那套矿物色画法放到全国——**一枚印章管一处地方，不是一首诗**，唐宋元 442 首落在 136 处（玉门关到儋州、上京草原到岭南），印章的颜色是朝代、印文是主题，一处压着好几首时印章更大、点开先出这一处的清单（长安四十四首、杭州二十四首、洛阳与汴京各十八首、金陵十七首）；诗名题签上写的是"地名 · N 首"，筛选之后数目会跟着改。归堆在判定层（`atlas/clusters.js`）：先按 0.12 度网格分，再把同名近邻并起来（格子边界会把长安切成两半），**分堆只取决于坐标、与数据表顺序无关**，所以印章 id 能直接当拾取标识；晕圈、杆与拾取体都是 `InstancedMesh`，印章要上到几百枚才不会被 draw call 拖死。条目从一百多首扩到一百七十多首时冒出一件只在密度上来后才看得见的事：**`place` 的首段就是这处地方的身份**，写成"长安市""长安里巷""长安南郊"会让长安散成三四枚印章，其中一枚还被邻近的乐游原抢了名字（于是《无题》的题签上写着"乐游原"），所以细部一律写在 `·` 之后（"长安·西市"），并由两条测试守着：**报同一个地名的诗必须落在同一枚印章上**、**一枚印章里的诗必须都报同一个首段**（否则印章刻谁的名字就成了众数碰运气——金陵那一枚曾因此刻着"石头城"）。这两条测试顺带翻出三处坐标错：《夏日绝句》的乌江偏了一百公里、《竹枝词》标夔州却落到一百四十公里外、《望天门山》挤进了乌江镇那一格。《静夜思》《锦瑟》这类**写作地历来无定说的，宁可不落点**（硬派一个地方就是编造），只进列表与检索；**落不下还有第二种理由 —— 地方认得出但在图幅之外**（这卷子西边止于玉门关一带，岑参的北庭、李颀的交河、王安石《明妃曲》的塞北毡城都在画外），两种都由同一条测试守着"必须在 place 里交代清楚"。放大到全国之后另有两件事必须重做：一是**色带不能一路加深**，横断山、青藏东缘一大片都在三千米以上，石青加深下去比海还暗、西半边会读成一片海湾，所以两千四以上改成越高越淡、最后淡进绢底（画里的"高则明、远则淡"），这条规则由 `atlas/palette.js` 的测试守着；二是**垂直夸张要按图幅重算**（省域图的 60 倍在这里会把秦岭拉成一百八十公里高，全图糊成一张皱铝箔，压到 30 倍）。一百多首一起写出题签就是一堵字墙，所以**全卷远看只出印章与古地名**，走近、点中、或那首诗在当前行迹上，诗名才铺开。数据按选集分册（`spots.js` 是最早那批，其后唐宋元各有 `spots-*-b/c.js`，末尾并成唯一的 `SPOTS`）；分批灌数据最容易犯的错是"同一首收了两遍、只是 id 起得不一样"，id 唯一性拦不住它，所以另有一条测试按"作者《篇名》"与正文前十四字各查一遍。元曲占 68 首，散曲的口气和唐诗宋词明显不同（睢景臣让一个乡下人把汉高祖的威仪拆了，萨都剌平视草原写乳酪和毡帘，无名氏《醉太平》张口就骂"堂堂大元"，卢挚与珠帘秀一赠一答成对收在扬州江上）。12 条行迹（李白／杜甫／王维／苏轼／白居易与江州／晚唐怅望／边塞一线／金陵怀古／南渡之路／南贬之路／元曲行路／宋金界上，共 170 站、覆盖 164 首），能搜作者、地名，也能搜诗句里的字；**空白分开的几个词是"并且"**（「李白 黄河」＝李白写到黄河的那几首），而且每条结果都写出命中在哪一处 —— 搜「明月」出来一条《山居秋暝》时，下面那行会标「句中…明月松间照…」，命中只在我写的赏析里时也如实标成「赏析」，不然读者会以为那是诗里的字。结果这一组紧跟在检索框下面：它原先排在整卷的最末，输一个词要往下滚一千多像素才看得见结果。另有一张**作者星图**（判定层 `atlas/starmap.js` + 画法 `scene/starChart.js`）：156 位作者按收录篇数分六等星，一等最亮（李白 20、王维 17、苏轼 16、杜甫 15），六等是只收一首的八十二位 —— 柱状图会把这条长尾压成一排看不清的短线，而**星等本来就是给"差几十倍"的量准备的**；同朝的连成一条旋臂（石青唐、石绿宋、赭石元），点一颗星看这位写过哪些地方，再点一首回地图上读它。星位只取决于数据（名次并列按字符序断、抖动取名字的散列而不是随机数），所以每次打开都一样，"星点不许叠在一起""篇数越多越亮"两条由测试守着。做这张图踩了三个坑，都记在代码注释里：把星等表整个摊进星对象会让它自带的 name（"一等星"）覆盖掉作者名；收边（拉回圆盘）必须放在每一轮推挤里做，否则最后统一收会把刚推开的两颗又按回一起；"最小径向间距"只能压内圈，一路压到外缘会把外圈那一大群六等星逐个推到边上、连成一圈钢丝。星盘的大小**两个方向各自算**：第一版取短边画正圆，1920 宽的屏上只铺开三成七、左右各空一大片，与"宽屏没适配"是同一个毛病；现在两轴各吃满自己的边、再互相压一道 1.6 的椭圆上限 —— 到 2:1 以内的屏两向都铺开六成以上，21:9 上铺不满是有意的代价（要铺满就得扁成一条带，旋臂会糊成三条横线），星图的面板也跟着地图那层一起有 1600／2200 两档大屏尺寸。这几条由 `test/starChart.test.js` 守着。
...
换翼S计划（2.5D）默认运行在 `http://127.0.0.1:4205`：

```bash
npm run dev:huanyi-s-jihua
```
