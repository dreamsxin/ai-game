...
// 按下到抬起位移小于这个（换算成场地格）算轻点，不算拖动。
// 5.5 格在手机上约等于 14 像素：透视换算把每像素值多少格抬高之后，
// 原来的 4 格只剩十来个像素，手指按下时轻轻一滚就够把弃翼吃掉。
export const TAP_SLOP = 5.5;
...
export function createInput(target = globalThis, { onPause, keyboard = true, pointer = true, scale } = {}) {
   ...
   *
   * 默认按「场地边长 / 画布边长」算，那是正交画法下的换算。
   * 2.5D 的表现层会用 `scale` 传进一份**按透视算的**换算（它知道船现在多深、
   * 一个像素在那个深度等于几格）——不接这一份，竖向拖动会比手指慢四成。
   */
  const pixels = () => {
    if (scale) {
      const custom = scale();
      if (custom && custom.sx > 0 && custom.sy > 0) {
        return { sx: custom.sx * DRAG_GAIN, sy: custom.sy * DRAG_GAIN };
      }
    }
  ...
  /**
   * 手势结束。轻点即弃翼，拖过一段距离的手势是走位，不该顺手把翅膀扔了。
   *
   * `lostpointercapture` 也走这一路：手机上长按被系统打断（来电、手势识别、页面切走）时
   * pointerup / pointercancel / pointerleave 三个都可能漏，而漏掉的后果是这根手指永远留在账上，
   * 后面每一次 pointermove 都被当成同一次拖动。
   * 反过来**不能只把它当「打断」直接销账**：Chrome 上 lostpointercapture 可能排在 pointerup 之前，
   * 那样一销账 pointerup 就找不到这根手指，轻点弃翼会整个失灵（真机上验出来过）。
   * 所以两个入口一视同仁，谁先到谁结账，后到的那个自然成了空操作。
   */
  const endTouch = (event) => {
    const touch = touches.get(event.pointerId ?? 0);
    if (!touch) return;
    touches.delete(event.pointerId ?? 0);
    if (touch.moved < TAP_SLOP && touch.time < TAP_TIME) press('jettison');
  };
  ...
  const listeners = [
    ...
    ['pointerup', endTouch, pointer],
    ['pointercancel', endTouch, pointer],
    ['lostpointercapture', endTouch, pointer],
  ...
  return {
    press,
    hold,
    /** 切出页面、面板弹出来的时候清账：不清的话回来船会自己一直往一个方向走。 */
    clear: clearHeld,
    /** 现在有哪几根手指按在屏幕上（client 坐标）。表现层拿它画拖动指示，只读。 */
    pointers() {
      return [...touches.values()].map((touch) => ({ x: touch.x, y: touch.y }));
    },
