...
//    "同名"这个条件很要紧：只按距离合会把乌江镇与八公里外的天门山并成一枚，
//    而只按名字合会把三处都叫"江南"的地方从润州一路并到姑苏。
//
// 印章刻的名字取自 `place` 的首段（见 shortPlace）。**同一枚印章里的诗必须报同一个首段** ——
// 这不是这一层能保证的，是数据要守的规矩，由 test/clusters.test.js 的两条测试盯着：
// 首段一散（"长安市""长安里巷"），长安就散成几枚；一格里混进两个身份（乌江镇与天门山），
// 印章刻谁的名字就成了众数碰运气 —— 金陵那一枚曾因此刻着"石头城"。
